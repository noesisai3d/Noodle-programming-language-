...
## Context
All expected outputs were written by hand from the draft spec before being compared with the bootstrap (TEST_STRATEGY.md). The first runs disagreed in five places.
...
5. **A test had its own bug** (`repeat until count is 0` from 7 in steps of 3 never ends). Fixed the test. This exposed that the bootstrap has no step limit, which is documented in BOOTSTRAP.md.

## Consequences
Two of these were design decisions, not just bug fixes (1 and 2). They're listed in QUESTIONS.md (Q12) and BEGINNER_LOG.md for the owner and for beginner testing.
